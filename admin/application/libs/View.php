<?	class View {
		...
		function addStyle($style, $after = '', $source = true, $off_dev_timestamp = false){
			if($source){
				return '<link rel="stylesheet" type="text/css" href="' . STYLE . $style .'.css'.( (DEVMODE === true && !$off_dev_timestamp ) ? '?t='.uniqid():'' ).'" '.$after.'/>'."\n\r";
			}else{
				return '<link rel="stylesheet" type="text/css" href="' . SSTYLE . $style .'.css'.( (DEVMODE === true && !$off_dev_timestamp  ) ? '?t='.uniqid():'' ).'" '.$after.'/>'."\n\r";
			}
		}
